...
## for computing shape features from 3D meshes.
## Reference: Code taken from echo-match repository and slightly modified.
...
def batch_render(compute_info, device, mesh, num_views, H, W):
    ...
    R, T = look_at_view_transform(dist=compute_info.cam_distance, azim=azimuth, elev=elevation, device=device, at=compute_info.cam_target.unsqueeze(0))
...
@dataclass
class DinoComputeOpts:
    cam_distance: float
    cam_target: torch.Tensor
    global_scale: float

def get_compute_opts(verts) -> DinoComputeOpts:
    ...
    return DinoComputeOpts(cam_distance=cam_distance, cam_target=bbox_center, global_scale=bbox_diag)

def compute_dino_features(compute_opts: DinoComputeOpts, verts, faces, num_views=100, H=512, W=512, tolerance=0.004):
