...
parser.add_argument(
    '--eig-method',
    type=str,
    choices=['sparse', 'numpy', 'scipy'],
    default='sparse',
    help='Eigendecomposition method: sparse (ARPACK), numpy (np.linalg.eigh), scipy (scipy.linalg.eigh)'
)
...
print(f"Eig method: {args.eig_method}")
opts = Options(device)
opts.eig_method = args.eig_method
...
err1 = main.run(sample1, output_folder="randomness_test_output", opts=opts)
err2 = main.run(sample1, output_folder="randomness_test_output", opts=opts)

print()
print(f"Error for {sample1.name}: {err1['mean']}")
print(f"Error for {sample1.name}: {err2['mean']}")